...
vector<pair<float, bool> > getRandomizedFloatVector(){
    const int MAX_PSM = 25000000;
    vector<pair<float, bool> > numbers(MAX_PSM);
    for(int i = 0 ; i < MAX_PSM; i++){
        numbers[i] = make_pair<float, bool>(get_random(), true);
    }
    return numbers;
}


int64_t testSort(vector<pair<float, bool> > &numbers){
...
int main(){
    ...
    for(int i = 0 ; i < 5; i++){
        vector<pair<float, bool> > numbers = getRandomizedFloatVector();
